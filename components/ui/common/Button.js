...
const Button = ({ children, className = "", variant="purple", ...rest }) => {
  ...
  return (
    <button className={`disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer rounded-md px-8 py-3 text-base border font-medium ${className} ${variants[variant]}`} {...rest}>
        {children}
    </button>
  )
}
